...
def main() -> None:
    ...
    parser.add_argument(
        "--usd", type=float, required=True, help="USD amount received after conversion",
    )
    parser.add_argument(
        "--fee", type=float, default=0.0, help="Conversion fee in USD (default: 0.0)",
    )
    parser.add_argument(
        "--rate-mid", type=float, default=None,
        help="Mid-market AUD/USD rate at time of deposit (e.g. from xe.com)",
    )
    ...
    if args.timestamp:
        try:
            ...
            print(
                "Error: --timestamp must be ISO 8601 format, e.g. 2026-03-30T14:00:00",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
