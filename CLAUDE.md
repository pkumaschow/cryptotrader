...
## Operational Rules (recovered from AI project memory, 2026-07-14)

### Never edit code directly on deployment targets

Never `sed`, write, or edit files on the Pi under `/opt/cryptotrader/` or `/opt/cryptotrader-staging/`. All fixes go in `~/project/cryptotrader/`, committed, then deployed via the GitLab CI/CD pipeline (manual deploy stage) or Ansible. SSH to the Pi is for inspection and log reading ONLY — direct changes are untracked, not reproducible, and get overwritten on the next deploy.

### Analysis: report only on production-active strategies

When analyzing strategies, filter to live trades only. The `trades` table has a `mode` column — use `WHERE mode='production'` to get exactly the live set. This beats filtering by strategy name (disabled strategies' historical trades AND test-mode runs of the active strategy both pollute a name filter). Config lives at `/opt/cryptotrader/config/settings.toml`. (Verified 2026-05-26: production = `bollinger` on BTC/ETH/SOL, 11 trades.)
