...
class KrakenWebSocket:
    """Public price feed, with reconnection and staleness tracking.

    Publishes each quote onto a queue for the trader. Reconnects with backoff,
    since a dropped feed is routine rather than exceptional — what matters is
    that `feed_healthy` reports the gap so orders can be suspended instead of
    placed against a stale price.
    """
    def __init__(self, pairs: list[str], price_queue: asyncio.Queue[PriceTick]) -> None:
        """Args:
        pairs: Pairs to subscribe to.
        price_queue: Queue receiving each tick.
        """
        self._pairs = pairs
    ...
    async def run(self) -> None:
        """Connect, subscribe and publish ticks until stopped.

        Reconnects on failure rather than raising: a feed drop should degrade
        trading, not end the process.
        """
        self._running = True
