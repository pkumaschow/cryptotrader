...
async def _poll_new_trades(trade_queue: asyncio.Queue[LogItem], db_path: str) -> None:
    """Push trades and refused orders written by the service into the TUI queue.

    Monitor mode has no in-process executor queue, so this poller is the only way
    either reaches the trade log when watching a running service.
    """
    # Seed cursors so we only pick up entries that arrive after monitor starts.
    last_trade_ts: datetime = datetime.now(UTC)
    last_reject_ts: datetime = last_trade_ts
    while True:
        await asyncio.sleep(3)
        try:
            new_trades = database.query_trades(
                db_path, since=last_trade_ts + timedelta(microseconds=1), read_only=True
            )
            new_rejects = database.query_rejected_orders(
                db_path, since=last_reject_ts + timedelta(microseconds=1), read_only=True
            )
            for trade in new_trades:
                last_trade_ts = trade.timestamp
            for reject in new_rejects:
                last_reject_ts = reject.timestamp
            # Interleave by time so a decline lands next to the trades around it.
            for item in sorted(list(new_trades) + list(new_rejects),
                               key=lambda x: x.timestamp):
                try:
                    trade_queue.put_nowait(item)
...
async def _run_monitor(hidden_panels: set[str]) -> None:
    ...
    trade_queue: asyncio.Queue[LogItem] = asyncio.Queue(maxsize=100)
...
async def _run(tui: bool, hidden_panels: set[str]) -> None:
    ...
    # Separate task on purpose: a REST round trip per resting order inside the
    # tick loop would stall tick consumption and leave gaps in the candles.
    # No-ops unless maker entries are enabled in production.
    maker_task = asyncio.create_task(trader.reconcile_maker_entries())
    ...
    if tui:
        ...
        try:
            await asyncio.gather(ws_task, trader_task, maker_task, health_task)
...
def main() -> None:
    ...
    Holds an exclusive lock on the database so a second instance cannot trade
    the same account. When the lock is already held, `--tui` degrades to a
    read-only monitor of the running service rather than refusing to start.
    """
