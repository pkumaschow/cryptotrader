"""Bollinger band breakout with optional trend filter and stop-loss.

Buys when a candle closes above the upper band; sells when one closes below the
middle band. That shape is worth understanding before trusting it: entries are
by definition at a local high and exits below the middle, so a full round trip
in a directionless market costs roughly one standard deviation plus fees. It
earns its keep in trends, not ranges.

`min_band_width_pct` suppresses breakouts in quiet markets, and the
higher-timeframe trend filter suppresses those bought into a falling market.
`stop_loss_pct` is the only exit that can realize a loss — the small-profit gate
blocks the others — so leaving it at zero means holding losers indefinitely.
"""
...
class BollingerStrategy(Strategy):
    # Entry price rides with the position flag: a refused SELL must restore both,
    # or the stop-loss has nothing left to measure against.
    """Breakout above the upper band, exit below the middle band.

    Entries are by construction at a local high and exits below the middle, so
    a complete round trip in a directionless market costs roughly one standard
    deviation plus fees. This earns its keep in trends, not ranges.
    """
    ...
    def evaluate(self, tick: PriceTick) -> Signal | None:
        ...
        Entry requires a close above the upper band, widening bands, a minimum
        band width, and a rising higher-timeframe trend if that filter is on.
        Exit is the stop-loss first, then a close below the middle band — but
        the small-profit gate blocks that second path when the move would not
        clear two legs of fees, which is why a zero stop means holding losers.
